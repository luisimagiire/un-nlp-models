...
class TagW2VConfig(KerasConfig):
...
class TagKerasW2V(KerasBaseModel):

    def __init__(self, configfile):
        self.name = "TagSpecificW2V"
        super().__init__(configfile)
        self.model_folder = self.get_model_folder()

    def get_config(self, config):
        return TagW2VConfig(config)
    ...
    def build_model_structure(self, opts=None):
        ...
        model = Model(inputed, tags_prediction)
        model.compile(optimizer='adam', loss={'tags': 'categorical_crossentropy'}, metrics=['acc'])
        print(model.summary())
        self.model = model
